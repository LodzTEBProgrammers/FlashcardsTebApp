...
public class JwtService : IJwtService
{
    ...
    public AuthenticationResponse CreateJwtToken(ApplicationUser user)
    {
        ...
        Claim[] claims = new[]
        {
            ...
            new Claim("jti", Guid.NewGuid().ToString()), // JWT unique ID
            new Claim("iat",
                DateTimeOffset.UtcNow.ToUnixTimeSeconds()
                    .ToString()), // Issued at (Unix timestamp)
            new Claim("email", user.Email), // Email of the user
            new Claim("name", user.PersonName) // Name of the user
        };
